...
namespace OnlineShop.AutomatedUITests
{
    ...
    {
        //собираемс€ использовать метод Dispose, чтобы закрыть окно chrome, открытое ChromeDriver, а также удалить его.
        ...
        public void Create_WhenExecuted_ReturnsCreateView()
        {
            ...
            //ѕосле перехода браузера к запрошенному URL-адресу будут заполнены свойства Title и PageSource объекта _driver
            Assert.Equal("Create - EmployeesApp", _driver.Title);
            Assert.Contains("Please provide a new employee data", _driver.PageSource);
        }

        //второй тест в котором мы провер€ем, по€вл€етс€ ли сообщение об ошибке на экране, если мы заполн€ем некоторые пол€ ввода, а не все из них, и нажимает кнопку "Create":
        [Fact]
